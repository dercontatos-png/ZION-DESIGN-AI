    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-4">
        <motion.div
          ...
          className="bg-zinc-900 border border-white/10 rounded-3xl p-6 sm:p-10 w-full max-w-lg text-center shadow-2xl relative overflow-hidden"
        >
          {/* Decorative light reflection */}
          <div className="absolute top-0 right-0 w-48 h-48 bg-amber-500/5 rounded-full blur-3xl pointer-events-none" />

          <div className="w-16 h-16 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6 text-amber-500 shadow-lg">
            <User size={32} />
          </div>
              ...
              <div className="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto pr-1">
                {activeClients.map((client) => (
                  <button
                    ...
                    className="w-full text-left p-4 rounded-2xl bg-zinc-950 border border-white/5 hover:border-amber-500/30 text-white flex items-center justify-between group transition-all"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      {client.avatarUrl ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-zinc-800 text-amber-500 flex items-center justify-center text-sm font-bold uppercase shrink-0">
                    ...
                    <ArrowRight
                      size={16}
                      className="text-zinc-500 group-hover:text-amber-400 group-hover:translate-x-1 transition-all shrink-0"
                    />
                  </button>
                ))}
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-zinc-900 to-zinc-950 border border-white/10 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 relative z-10">
          {currentClient.avatarUrl ? (
            ...
            <div className="w-16 h-16 rounded-2xl bg-zinc-800 text-amber-500 flex items-center justify-center text-xl font-bold uppercase">
              {currentClient.name.substring(0, 2)}
            </div>
          )}
          <div>
            <span className="bg-amber-500/10 text-amber-400 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider border border-amber-500/20">
      ...
      {activeClientTab === "overview" && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {/* Card 1: Seu Plano */}
            <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5 relative overflow-hidden">
            ...
            <div className="bg-zinc-900 border border-white/5 rounded-2xl p-5 relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 mt-2">
                {currentClient.paymentStatus === "Em dia" ? (
                  ...
                  <span className="inline-flex items-center gap-1 bg-amber-500/10 text-amber-400 text-xs px-2.5 py-1 rounded-full font-bold">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-zinc-900 border border-white/5 rounded-3xl p-5 sm:p-6 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                  <CheckSquare size={16} className="text-amber-500" /> Atividades Recentes
                </h3>
              ...
              {clientTasks.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center p-6 text-center border border-dashed border-white/5 rounded-2xl">
            ...
            <div className="bg-zinc-900 border border-white/5 rounded-3xl p-5 sm:p-6 flex flex-col">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2 mb-4">
                <Calendar size={16} className="text-amber-500" /> Próximos Compromissos & Postagens
              </h3>
      ...
      {activeClientTab === "deliveries" && (
        <div className="bg-zinc-900 border border-white/5 rounded-3xl p-5 sm:p-6 space-y-6">
          <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                <span className="text-xs font-bold uppercase text-zinc-400 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" /> Em Execução ({clientTasks.filter((t) => t.status === "doing").length})
                </span>
              </div>
              <div className="space-y-3">
                {clientTasks.filter((t) => t.status === "doing").map((t) => (
                  <div key={t.id} className="p-4 rounded-xl bg-zinc-950 border border-amber-500/10 hover:border-amber-500/20 transition-colors">
                    <p className="text-xs font-bold text-white leading-snug">{t.title}</p>
                    {t.dueDate && (
                      <div className="mt-3 flex items-center gap-1.5 text-[10px] text-amber-500">
                        <Clock size={12} /> Limite: {formatTaskDueDate(t.dueDate)}
                      </div>
                    )}
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {clientTasks.filter((t) => t.status === "done").map((t) => (
                  <div key={t.id} className="p-4 rounded-xl bg-zinc-950/40 border border-emerald-500/10 text-zinc-500">
      ...
      {activeClientTab === "finance" && (
        <div className="bg-zinc-900 border border-white/5 rounded-3xl p-5 sm:p-6 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="bg-zinc-950 border border-white/5 rounded-2xl px-4 py-3 flex items-center gap-3 self-start sm:self-center shrink-0">
          ...
          <div className="bg-zinc-950 rounded-2xl overflow-hidden border border-white/5">
                    ...
                    <div key={t.id} className="p-4 sm:px-6 flex items-center justify-between gap-4 hover:bg-white/[0.01] transition-colors">
                      ...
                      <div className="flex items-center gap-4 shrink-0 text-right">
                        ...
                        {t.status === "pago" ? (
                          ...
                          <span className="inline-flex items-center gap-1 bg-amber-500/10 text-amber-400 text-[10px] font-black uppercase px-2 py-0.5 rounded">
      ...
      {activeClientTab === "calendar" && (
        <div className="bg-zinc-900 border border-white/5 rounded-3xl p-5 sm:p-6 space-y-6">
          ...
          {clientEvents.length === 0 ? (
            <div className="text-center p-12 border border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center">
      ...
      {activeClientTab === "notes" && (
        <div className="space-y-6">
          <div className="bg-zinc-900 border border-white/5 rounded-3xl p-5 sm:p-6">
          ...
          {clientNotes.length === 0 ? (
            <div className="text-center p-12 border border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center bg-zinc-900/40">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {clientNotes.map((note) => (
                <div
                  ...
                  className="bg-zinc-900 border border-white/5 hover:border-amber-500/20 rounded-2xl p-5 cursor-pointer flex flex-col justify-between h-52 group transition-all"
                >
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[10px] font-black uppercase bg-amber-500/10 text-amber-400 px-2.5 py-0.5 rounded-full border border-amber-500/10">
                        {note.type || "Briefing"}
                      </span>
                    ...
                    <h4 className="font-bold text-sm text-white group-hover:text-amber-400 line-clamp-1 transition-colors">
                      {note.title}
                    </h4>
                  ...
                  <span className="text-[10px] font-bold text-amber-500 flex items-center gap-1 mt-3 group-hover:underline">
          ...
          {selectedNote && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <motion.div
                ...
                className="bg-zinc-900 border border-white/10 rounded-3xl p-6 w-full max-w-2xl max-h-[85vh] overflow-y-auto shadow-2xl flex flex-col"
              >
