  ...
  return (
    ...
    >
      <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
        ...
        {/* Slider line & handle */}
        <div 
          className="absolute inset-y-0 w-0.5 bg-[#a855f7] pointer-events-none z-10 flex items-center justify-center"
          style={{ left: `${sliderPos}%`, transform: 'translateX(-50%)' }}
        >
          <div className="w-8 h-8 rounded-full bg-black shadow-xl flex items-center justify-center border-2 border-violet-500">
            <MoveHorizontal size={14} className="text-violet-400" />
          </div>
        </div>
        ...
        <div className="absolute top-4 right-4 px-3 py-1.5 bg-violet-500/20 backdrop-blur-md rounded-lg border border-violet-500/50 text-violet-400 font-black tracking-widest text-[10px] uppercase z-10 pointer-events-none">Depois (Refinado)</div>
      </div>
    </div>
  );
}
